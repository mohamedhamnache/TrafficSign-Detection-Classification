...
def findSignsOnFrame(image, contours, threshold, distance_theshold):
    coordinate = None
    sign = None
    signList = []
    for c in contours:
        ...
        if is_sign and distance > distance_theshold:
            ...
            coordinate = [(left-2,top-2),(right+3,bottom+1)]
            sign = cropSign(image,coordinate)
            signList.append((sign, coordinate))
    return signList
...
#         cv2.imwrite('results/'+str(count)+'_'+text+'.png', sign)
...
def localizationVersion2(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    original_image = image.copy()
    binary_image = preprocess_image(image)
    ...
    #signs, coordinates = findSigns(image, contours, similitary_contour_with_circle, 15)
    # 
    signList = findSignsOnFrame(original_image, contours, similitary_contour_with_circle, 15)
    resList = []
    for sign, coordinate in signList:
        text = ""
        sign_type = -1
        i = 0
        ...
        if sign is not None:
            sign_type = getLabel(model, sign)
            sign_type = sign_type if sign_type <= 15 else 15
            text = SIGNS[sign_type]
            cv2.imwrite('results/'+str(count)+'_'+text+'.png', sign)
        ...
        if sign_type > 0 and sign_type != current_sign_type:        
            ...
            font = cv2.FONT_HERSHEY_PLAIN
            cv2.putText(original_image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
        resList.append((coordinate, sign_type, text))
    return resList, original_image
...
def main(args):
    ...
    while True:
        ...
        signList,  image= localizationVersion2(frame, args.min_size_components, args.similitary_contour_with_circle, model, count, current_sign)
        ...
        for coordinate, sign_type, text in signList:
            ...
            if sign_type > 0 and (not current_sign or sign_type != current_sign):
                ...
                position = [count, sign_type if sign_type <= 15 else 15, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
                ...
                if sign_type > 0:
                    ...
                    position = [count, sign_type if sign_type <= 15 else 15, left, top, right, bottom]
                    cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 1)
                    font = cv2.FONT_HERSHEY_PLAIN
                    cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
                elif current_sign:
                    position = [count, sign_type if sign_type <= 15 else 15, tl[0], tl[1], br[0], br[1]]
...
if __name__ == '__main__':
    ...
    args.file_name='images/imn_video.mp4'
    main(args)
